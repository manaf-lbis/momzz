/**
 * Advanced Search & Fuzzy Matching Algorithm for Momzz
...
/**
 * Fast Damerau-Levenshtein distance calculation (handles insertions, deletions,
 * substitutions, and adjacent transpositions like "teh" -> "the").
 */
...
/**
 * Generates character n-grams (default: bigrams).
 */
 ...
 * Exceptional for partial fuzzy substring and typo matching.
 */
...
/**
 * Common automotive and garage synonyms / abbreviations map
 */
...
export function calculateMatchScore(
  ...
  if (qTokens.length > 0) {
    ...
    for (const qToken of qTokens) {
      ...
      // Check against each title token
      ...
      // If token not matched in title tokens, check if query token is a substring of the title
      if (bestTokenScore === 0 && normTitle.includes(qToken)) {
        bestTokenScore = 0.7;
      }

      // Check category name for the token
    ...
    // If all tokens matched with high confidence
 ...
 * Items with scores above the threshold (default: 200) are returned in descending score order.
 */
export function advancedSearch<T>(
  ...
  // Sort descending by score; if tied, sort shorter titles first (tighter matches)
  scored.sort((a, b) => {
    if (b.score !== a.score) {
 ...
 * Threshold: 0.82 (82% similarity or identical normalized/stripped representation).
 */
export function findDuplicateCandidates<T>(
  ...
  for (const item of existingItems) {
    ...
    // Check exact normalized equality
    ...
    // Check stripped equality (ignoring all spaces and punctuation)
    ...
    // Check reordered word tokens (e.g. "Toyota Oil Filter" vs "Oil Filter Toyota")
    ...
    // Check Levenshtein edit similarity
