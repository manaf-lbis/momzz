...
import { Car, Search, CheckCircle2, Clock, Wrench, Calendar } from 'lucide-react';
...
export const TrackServicePage: React.FC = () => {
  ...
  return <main className="min-h-screen bg-zinc-100 dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 px-4 py-10"><div className="max-w-xl mx-auto space-y-6">
    ...
    {jobs.map((job) => {
      const done = job.tasks.filter((task) => task.status === 'COMPLETED').length;
      const progress = job.tasks.length ? Math.round((done / job.tasks.length) * 100) : 0;
      const isReady = job.status === 'COMPLETED' || progress === 100;
      const status = isReady ? 'Ready for Delivery' : progress ? 'In Progress' : 'Received';
      const deliveryInfo = job.expectedDeliveryDate ? getDeliveryStatusInfo(job.expectedDeliveryDate, isReady) : null;
      return <section key={job.id} className="industrial-card rounded-2xl p-5 space-y-4">
        <div className="flex items-start justify-between gap-3">
          <div>
            <h2 className="font-bold uppercase">{job.vehicleName} {job.vehicleColor && <span className="text-amber-500">({job.vehicleColor})</span>}</h2>
            <p className="text-xs font-mono text-zinc-500">Service Date: {serviceDate(job.createdAt)}</p>
            {job.expectedDeliveryDate && deliveryInfo && (
              <p className="text-xs font-mono font-bold text-amber-600 dark:text-yellow-400 mt-1 flex items-center gap-1">
                <Clock className="w-3.5 h-3.5" /> Expected Delivery: {formatDeliveryDate(job.expectedDeliveryDate)}
              </p>
            )}
          </div>
          <span className={`text-[10px] font-mono font-bold uppercase rounded-full px-2.5 py-1 ${status === 'Ready for Delivery' ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' : 'bg-amber-500/10 text-amber-600 dark:text-yellow-400'}`}>{status}</span>
        </div>
        <div className="flex justify-between text-xs font-bold"><span>Task progress</span><span>{done}/{job.tasks.length} complete</span></div>
        <div className="h-2 rounded-full bg-zinc-200 dark:bg-zinc-800 overflow-hidden"><div className="h-full bg-emerald-500 transition-all" style={{ width: `${progress}%` }} /></div>
        <ol className="space-y-2">{job.tasks.map((task) => <li key={task.id} className="text-sm flex gap-2 items-center">{task.status === 'COMPLETED' ? <CheckCircle2 className="w-4 h-4 text-emerald-500" /> : <Clock className="w-4 h-4 text-amber-500" />}{task.title}</li>)}</ol>
        <p className="text-xs text-zinc-500 flex gap-1 items-center"><Wrench className="w-3 h-3" /> Updates appear automatically.</p>
      </section>;
    })}
  </div></main>;
};
